#!/usr/bin/env bash
# FAZA 16f - ile kosztuje tor obrazu na biegu jalowym.
#
...
OUT=results/idle-with-projector.jsonl
SEC=${SEC:-120}
SETTLE=${SETTLE:-25}
DEV=DP-1
...
echo "=== FAZA 16f start: SEC=$SEC SETTLE=$SETTLE ==="
...
ustaw(){   # $1 = nazwa ramienia
  case "$1" in
    ...
    off)   kd output.$DEV.mode.$MODE_BASE; sleep 2; kd --dpms off ;;
    *) return 1 ;;
  esac
  sleep 3; }

for pass in 1 2; do
  if [ "$pass" = 1 ]; then V="hz144 hz60 off"; else V="off hz60 hz144"; fi
  for v in $V; do
    ustaw "$v" || { echo "  SETFAIL $v"; continue; }
    echo "  $v/p$pass: tryb=$(tryb) dpms=$(dpmsstan)"
