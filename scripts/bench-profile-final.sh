#!/bin/bash
# Faza 10 krok 4: zlozenie wszystkiego. ASPM performance + vo -75 mV + cap SCLK.
# Uzycie: ./bench-profile-final.sh 2200 2000     (lista maksymalnych SCLK; 3045 = bez capa)
set -u
cd "$(dirname "$0")"
...
OUT=results/profile-final.jsonl
...
set_all(){ local mhz=$1
  ...
  echo "s 1 $mhz" > "$OD" || { echo "s 1 odrzucony"; return 1; }
  ...
  echo "  zapisano vo=$MV sclk=$mhz | odczyt vo=${gv}mV sclk=${gs}MHz perf=$(cat $PL) aspm=$(polstate) l1=$(cat $LNK/l1_aspm)"
  [ "$gv" = "$MV" ] && [ "$gs" = "$mhz" ] || { echo "  ROZBIEZNOSC: sterownik nie przyjal obu wartosci"; return 1; }
}
...
for mhz in "$@"; do
  tag="final-aspm-uv75-sclk${mhz}"
  echo "=== $tag ==="
  set_all "$mhz" || { echo "$tag SETFAIL" | tee -a $OUT; continue; }
  p=$(ppl "$tag"); echo "PPL $tag EN=$p" | tee -a $OUT
  kill_srv; cool
  SRV_LOG="logs/f10-$tag.log" ./srv.sh $BASE $SPEC $REAS || { echo "SRVFAIL $tag" | tee -a $OUT; continue; }
  for rep in 1 2; do
