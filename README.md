...
| [Turning reasoning on erased every one of those errors](docs/context-and-quality.md#reasoning-on-the-errors-above-do-not-appear-at-all) | 64/64 against 19/64 on the same 120K material, both arms on one server with the shipped profile. It costs 17.3x the median latency and ~2500 reasoning tokens per counting answer, and the sample only supports "did not occur", not "cannot occur". |
| [Effort `low` scores the same for a third of the tokens, unevenly](docs/context-and-quality.md#effort-low-scores-the-same-costs-a-third-as-much-and-does-it-unevenly) | 64/64 again on the same material, 2.9x fewer output tokens and 2.2x less wall clock. But it changes nothing on retrieval, and on counting it is bimodal rather than short: 6 of 32 queries still ran to ~2900 tokens, so the median drops 6x while the p90 stays at 64 s. Changing the level also invalidates the entire prompt cache, because the template prepends the instruction to the system block. |
| [Running out of reasoning budget closes the thought, it does not cut the answer](docs/context-and-quality.md#exhausting-the-reasoning-budget-closes-the-thought-it-does-not-cut-the-answer) | At 512 instead of 8192, counting still scored 8/8 on 4.9x fewer tokens, every request finished on `stop`, and no answer came back empty or truncated - the server closes the thinking block itself. The enumeration inside it is cut off at paragraph 119 of 800, which means the completeness of that written list is not what repairs the arithmetic a reasoning-off run gets wrong. |
